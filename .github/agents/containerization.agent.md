...
- `aks-project/pipeline-repo: gambtho/grpc-retry-fun`
...
- Trigger ONLY on `workflow_dispatch` with the following required string inputs (and their defaults):
  - `cluster-name` (default: `cluster-b`)
  - `resource-group` (default: `pr-314-test-rg`)
  - `namespace` (default: `new-project`)
...
- Ensure all manifests target namespace: `new-project`
